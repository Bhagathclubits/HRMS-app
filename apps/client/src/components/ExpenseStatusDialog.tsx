...
import { ExpenseStatus } from "server/src/trpc/routes/expense-status/get-many";
...
export type expenseStatusProps = {
  expenseId: number;
  variant: "admin" | "employee";
  asyncList: AsyncListContextValue;
};

const ExpenseStatusDialog = (props: expenseStatusProps) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await client.expense.adminUpdate.mutate({
        id: props.expenseId,
        remarks: remarks,
        statusId: statusId,
      });
  ...
  React.useEffect(() => {
    (async () => {
      const status = await client.expenseStatus.getMany.mutate();
